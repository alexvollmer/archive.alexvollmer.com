...
comments: 
- :author: Ted Naleid
  :date: 2009-05-10 18:53:17 -07:00
  :body: |-
    Interesting post Alex, thanks for getting into the details.
    
    What percentage of devs would you say are "pro-Git" (or at least savvy and interested in using a DVCS)?
    
    How do you have CI integrated into this?  Is it only running on the authoritative box, or is it also monitoring all of the developer instances?
    
    At the last place I worked at, I was able to convince everyone to move to Mercurial but we didn't quite go as far with the workflow as you show above.
    
    I was the only dev (out of ~12 people) who had any significant DVCS experience, so we had a an authoritative repo that anyone could push/pull from.  People then just had local copies of that repo on their machine.
    
    If people wanted to share, there's a command line tool in mercurial (hg serve) that lets you start up a webserver that others can pull from (or you can send patch files).  They could also make local repo clones if desired.
    
    We also had a separate "sandbox" repo server that people could create apps in and push to if desired, but it wasn't a normal part of most people's workflow.
    
    That felt easier to me than going the full route with the relative lack of DVCS experience on the team.  Getting them all to do that extra "hoop-jumping" didn't feel realistic.  
    
    I'm actually not all that sure that it's even worth it in an in-house development project with a relatively small team of developers.
    
    I'll be interested to hear in the future if everyone adapts to the new workflow, or if it gets adapted/streamlined as people get more comfortable.
  :url: http://naleid.com/blog
  :id: 
- :author: alex
  :date: 2009-05-11 02:26:36 -07:00
  :body: |-
    I'd say that about 50-60% of the devs are on-board with the Git conversion. However, most of us have used Git primarily as a wrapper over Subversion which is a sort-of degenerate version of "normal" Git usage.
    
    As for CI, we're using Hudson to build our Java code. The latest version has a Git plugin that seems to work well. We just point it at our authoritative repos using the anonymous, read-only git:// protocol.
    
    There's a cool tool called <a href="http://github.com/technomancy/gitjour/tree/master" rel="nofollow">Gitjour</a> which allows you to export, search and find git repositories over Bonjour/DDNS. It's a fantastic way to share code at meetups and conferences.
    
    We're just starting the Git-conversion so it remains to be seen how we end up using. The developer repos may never really catch fire if folks just share patches via email or Gitjour.
...
Gitweb has a ton of functionality—including all kinds of search capability, different views (by tag, by commit, by tree) and even serves up an Atom feed of changes—but it is definitely a <span class="snarky-comment">designed-by-developers-for-developers</code>. (NB: In case you missed the markup, that is considered an epithet on my team). 
...
![98EE3AE7-C4F4-4D1D-B49B-DCC230EA7459.jpg](/uploads/2009/05/98ee3ae7-c4f4-4d1d-b49b-dcc230ea7459.jpg)
...
`</pre>
Now because there are number of steps involved here, we wrote a little command-line tool (as a RubyGem) that takes care of these steps all in one go. Right now, it's _very_ specific to our setup at Evri, but I can see how we might extract a common tool (oh boy…another side-project…)
...
![git-cycle.png](/uploads/2009/05/git-cycle.png)
